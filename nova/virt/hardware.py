...
def _numa_fit_instance_cell(host_cell, instance_cell, limit_cell=None):
    ...
    if instance_cell.cpu_pinning_requested:
        ...
        mem_curr = host_state.metrics['numa.membw.current'].value[str(host_cell.id)]
        mem_total = host_state.metrics['numa.membw.max'].value[str(host_cell.id)]
        utili = mem_curr / mem_total
        if utili > .4:
            return None
